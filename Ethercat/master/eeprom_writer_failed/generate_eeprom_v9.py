...
# Default sizes (will be overridden by your C code, but needed for safety)
SM2_SIZE = 300 
SM3_SIZE = 540
...
def build_eeprom():
    ...
    # PDI Config (Byte 2-3): 0x0000 (SPI Mode 0)
    eeprom[2] = 0x00
    eeprom[3] = 0x00

    # *** CRITICAL FIX: MAILBOX PROTOCOL (Byte 56 / Word 0x1C) ***
    # 0x0004 = CoE (CANopen over EtherCAT)
    # If this is 0, Master disables Mailbox and cannot read sizes!
    eeprom[56] = 0x04
    eeprom[57] = 0x00

    # 3. DATA AREA (Offset 128) - "The Safe Zone"
    offset = 128
